...
if_wan_in=1 # 1: GigaEthernet0, 3: GigaEthernet2, 1073741823(0x3FFFFFFF): IXの自生成パケット

bytes_in=0
bytes_out=0
goflow -kafka=false -logfmt=json -nf=false | while read -r line; do  
  # フローサンプルを加算して
  if [ "$(echo "$line" | jq -r '.InIf')" == "$if_wan_in" ]; then
    # 下り
  ...
  # 100秒ごとにLokiに送信 trrafic bits/s = Bytes * 8bits * 1000samples / 100sec
